...
After an approved private upload, verify channel ownership, privacy, and all
four subtitle languages with `scripts/youtube_verify_upload.py`.

## Remote-write gate

The private test upload completed on 2026-09-25 and was verified through the
YouTube API with EN/RU/TR/ID caption tracks. The remote video id is stored only
in the gitignored recovery receipt under `.runtime/marketing/`.

This run is not approved for public or unlisted publication. The local OAuth
token includes `youtube.force-ssl` for automated caption-track upload.
